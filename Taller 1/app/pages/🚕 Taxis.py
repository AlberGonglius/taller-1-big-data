...
Tipo = st.radio('Que quieres ver?', ('Heatmap', 'Top destinos', 'Km/año', 'Viajes más costosos'))
...
years = st.multiselect("Elige un año", l_year, [2020])
...
if not years:
    ...
    ################## compara km recorridos por año #################################
        ...
        with st.spinner('Calculando...'):
            ...
            fig, ax = plt.subplots()
            sns.barplot(sumkmy, x="pickup_year", y="distancia").set(title='Cantidad de kilometros recorridos por año')
    ...
    ################## cálculo destinos #################################
        ...
        with st.spinner('Calculando...'):
            ...
            sns.barplot(dest, y="DO_Zone", x="conteo").set(title='Destinos más populares (# viajes)')
